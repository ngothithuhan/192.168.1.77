...
class TableGiaiToaCCQ extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pages: null,
    ...
    async loadData(pagesize, page, keySearch, sortSearch) {
        ...
        RestfulUtils.post('/balance/getlistsemastunblock', { ...obj, page, keySearch, sortSearch }).then(resData => {
            if (resData.EC == 0) {
                that.setState({
    ...
    search() {
        ...
        //console.log('sefl.state.pagesize', this.state.pagesize)
        if (mssgerr == '') {
            RestfulUtils.post('/balance/getlistsemastunblock', { custodycd: this.state.CUSTODYCD.value, pagesize: this.state.pagesize, language: this.props.language, OBJNAME: this.props.OBJNAME }).then((resData) => {
                if (resData.EC == 0) {
    ...
    async reloadTable() {
        let that = this;
        await RestfulUtils.posttrans('/balance/getlistsemastunblock', { pagesize: that.state.pagesize, page: that.state.page, keySearch: that.state.keySearch, sortSearch: that.state.sortSearch, language: that.props.language, OBJNAME: this.props.OBJNAME }).then((resData) => {
            if (resData.EC == 0)
...
const decorators = flow([
    ...
    translate('TableGiaiToaCCQ')
]);

module.exports = decorators(TableGiaiToaCCQ);
