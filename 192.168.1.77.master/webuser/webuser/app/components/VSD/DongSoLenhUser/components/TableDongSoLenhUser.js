...
class TableDongSoLenhUser extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pages: null,
    ...
    refresh = () => {
        ...
        RestfulUtils.posttrans('/order/getlistCloseOrderManual', { pagesize: this.state.pagesize, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
            if (resData.EC == 0) {
    ...
    async loadData(pagesize, page, keySearch, sortSearch, columns) {
        let that = this;
        await RestfulUtils.post('/order/getlistCloseOrderManual', { pagesize, page, keySearch, sortSearch, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
                ...
                <div className="col-md-12" >
                    <ReactTable
                        columns={[
                            ...
                            {
                                Header: props => <div className=""><b>{this.props.strings.ISSIP}</b></div>,
                                id: "ISSIP",
                                accessor: "ISSIP",
                                width: 100,
                                Cell: ({ value }) => (
...
TableDongSoLenhUser.defaultProps = {
};
...
const decorators = flow([
    ...
    translate('TableDongSoLenhUser')
]);
module.exports = decorators(TableDongSoLenhUser);
