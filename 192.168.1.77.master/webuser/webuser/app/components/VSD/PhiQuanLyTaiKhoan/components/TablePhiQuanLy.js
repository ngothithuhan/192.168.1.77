...
class TablePhiQuanLy extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pages: null,
    ...
    refresh = () => {
        ...
        RestfulUtils.posttrans('/order/getlistmanagerfee', { pagesize: this.state.pagesize, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
            if (resData.EC == 0) {
    ...
    async loadData(pagesize, page, keySearch, sortSearch, columns) {
        let that = this;
        await RestfulUtils.post('/order/getlistmanagerfee', { pagesize, page, keySearch, sortSearch, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
                ...
                <div className="col-md-12" >
                    <ReactTable
                        columns={[
                            ...
                            {
                                Header: props => <div className=""><b>{this.props.strings.MANAGERFEEAMT}</b></div>,
                                id: "MANAGERFEEAMT",
...
const decorators = flow([
    ...
    translate('TablePhiQuanLy')
]);
module.exports = decorators(TablePhiQuanLy);
