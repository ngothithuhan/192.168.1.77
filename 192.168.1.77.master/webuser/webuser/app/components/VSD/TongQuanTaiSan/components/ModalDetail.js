...
import { SRTYPE_NR, SRTYPE_NS, SRTYPE_SW, COLORSW, COLORNR, COLORNS, amt_width, qtty_width, symbol_width, date_width } from 'app/Helpers'
...
class ModalDetail extends React.Component {
	...
	render() {
		const { isOpen, onClose, dataDetail, catalogDetails, detailColumn } = this.props
		return (
			<Modal className="custom_modal" show={isOpen} onHide={onClose}>
				<div className="titleDetail">
					<div className="titleDetail_left">
						<div>
							<span>{this.props.strings.categoryCode}</span>&nbsp;
								<span className="titleDetail--info">
									{catalogDetails.categoryId}
							</span>
						</div>
					</div>
