...
class TableAddCreateQLQM extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data: [
    ...
    handleChangeALL(evt) {
        ...
        if (evt.target.checked) {
            that.state.data.map(function (item) {
                if (!that.state.selectedRows.has(item.CODEID)) {
                    that.state.unSelectedRows.push(item.CODEID);
                    that.state.selectedRows.add(item.CODEID);
                }
            })
    ...
    handleChange(row) {
        ...
        if (!this.state.selectedRows.has(row.original.CODEID))
            this.state.selectedRows.add(row.original.CODEID);
        else {
            this.state.selectedRows.delete(row.original.CODEID);
        }
    ...
    fetchData(state, instance) {
        ...
        if (this.state.firstRender) {
            ...
            RestfulUtils.posttrans('/fund/getlist', data).then((resData) => {
        ...
        if (this.state.selectedRows.size == 0) toast.error(this.props.strings.warningchooserecord, { position: toast.POSITION.BOTTOM_RIGHT })
            ...
            this.state.selectedRows.forEach((key, value, set) => {
                ...
                new Promise((resolve, reject) => {
                    ...
                    let data = this.state.data.filter(e => e.CODEID === value);
                    let success = null;
                    let datadelete = {
                        data: data[0],
                        p_language: this.props.lang,
                    ...
                    resolve(RestfulUtils.posttrans('/fund/delete', datadelete)
                        .then(res => {
                            i += 1
                ...
                <div className="col-md-12" >
                    <ReactTable
                        columns={[
                                ...
                                Cell: (row) => (
                                    <div>
                                        <Checkbox style={{ textAlign: "center", marginLeft: "11px", marginTop: "-14px" }}
                                            checked={that.state.selectedRows.has(row.original.CODEID)}
                            ...
                            {
                                Header: props => <div className="wordwrap" id="lbCODEID">{this.props.strings.SYMBOL}</div>,
                                id: "SYMBOL",
                                accessor: "SYMBOL",
                                width: 143,
                                Cell: ({ value }) => (
                            ...
                            /*
                            {
                                Header: props => <div className="header_table_pad_3" id="lbNAME">{this.props.strings.vfmname}</div>,
                                id: "NAME",
                                accessor: "NAME",
                                width: 200,
                                Cell : ({value}) =>(
                        ...
                        */
                            {
                                Header: props => <div className="wordwrap" id="lbNAME_VN">{this.props.strings.NAME_VN}</div>,
                                id: "NAME_VN",
                                accessor: "NAME_VN",
                                width: 500,
                                Cell: ({ value }) => (
                            ...
                            {
                                Header: props => <div className="wordwrap" id="lbNAME_EN">{this.props.strings.NAME_EN}</div>,
                                id: "NAME_EN",
                                accessor: "NAME_EN",
                                width: 430,
                                Cell: ({ value }) => (
                            ...
                            /*
                            {
                                Header: props => <div className="wordwrap" id="lbSETTLEACCTNO">{this.props.strings.bankaccno}</div>,
                                id: "SETTLEACCTNO",
                                accessor: "SETTLEACCTNO",
                                width: 100,
                                Cell : ({value}) =>(
                                    <div className="col-left" id={"lb"+value}>{value}</div>
                                )
                            ...
                            {
                                Header: props => <div className="header_table_pad_3" id="lbSETTLEBANK">{this.props.strings.bankacc}</div>,
                                id: "SETTLEBANK",
                                accessor: "SETTLEBANK",
                                width: 200,
                                Cell : ({value}) =>(
                            ...
                            {
                                Header: props => <div className="wordwrap" id="lbBLOCKACCTNO">{this.props.strings.acchold}</div>,
                                id: "BLOCKACCTNO",
                                accessor: "BLOCKACCTNO",
                                width: 100,
                                Cell : ({value}) =>(
                                    <div className="col-left" id={"lb"+value}>{value}</div>
                                )
                            ...
                            {
                                Header: props => <div className="header_table_pad_3" id="lbBLOCKBANK">{this.props.strings.bankacchold}</div>,
                                id: "BLOCKBANK",
                                accessor: "BLOCKBANK",
                                width: 200,
                                Cell: ({ value }) => (
...
TableAddCreateQLQM.defaultProps = {

    strings: {
        custid: 'Mã Quỹ',
        custiodycd: 'Tên CCQ',
        fullname: 'Tên tiếng việt',
        idcode: 'Tên tiếng Anh',
        iddate: 'Giấy phép lập quỹ',
        opendate: 'Ngày cấp',
        idplace: 'Nơi cấp',
        address: 'Số TK Tiền Thanh toán',
        custtype: 'Ngân hàng mở TK thanh toán',
        status: 'Số TK tiền phong tỏa',
        pageText: 'Trang',
...
const decorators = flow([
    ...
    translate('TableAddCreateQLQM')
]);

module.exports = decorators(TableAddCreateQLQM);
