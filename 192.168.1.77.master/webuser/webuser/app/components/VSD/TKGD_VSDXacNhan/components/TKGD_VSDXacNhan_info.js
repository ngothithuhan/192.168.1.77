...
import { showNotifi } from 'app/action/actionNotification.js';
 ...
 class TKGD_VSDXacNhan_info extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      collapse: {
 ...
 onValueChange(type, data) {
  ...
  async submitUser  () {
    ...
    RestfulUtils.post(api,this.state.datauser)
      .then((res)=>{
        if(res.data.EC==0) {
          datanotify.type = "success";
          datanotify.content = "Thành công";
...
TKGD_VSDXacNhan_info.defaultProps = {
...
const decorators = flow([
  ...
  translate('TKGD_VSDXacNhan_info')
]);

module.exports = decorators(TKGD_VSDXacNhan_info);
