...
class TableIN extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (!nextProps.isClearCheck1)
            this.setState({
                //    data1: nextProps.dataIN,
                data: nextProps.dataIN,
                //  loaded: true,
            })
        ...
        if (evt.target.checked) {
            that.state.data.map(function (item) {
                if (!that.state.selectedRows.has(item.TLID)) {
                    ...
                    that.props.datacheckedin(item, 'checked')
                }
            })
            ...
            that.state.data.map(function (item) {
                ...
                that.props.datacheckedin(item, 'unchecked')
    ...
    handleChange(row) {
        ...
        if (!this.state.selectedRows.has(row.original.TLID)) {
            ...
            this.props.datacheckedin(row.original, 'checked')
        }
        else {
            this.state.selectedRows.delete(row.original.TLID);
            this.props.datacheckedin(row.original, 'unchecked')
        }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Checkbox id="ckAlliN" checked={that.state.checkedAll} style={{ paddingLeft: "20px", paddingTop: "10px" }} onChange={that.handleChangeALL.bind(that)} inline >{this.props.strings.checkall}</Checkbox>
...
module.exports = decorators(TableIN);
