...
class ThongTinQuyDTDK extends React.Component {
  constructor(props) {
    super(props);
    var date = new Date();
    this.state = {
  ...
  render() {
    ...
    let symbolname = sessionInfo.FULLNAME ? (sessionInfo.FULLNAME + '(' + this.state.CODEID.label + ')') : '';
    var title = this.props.strings.infoFund + symbolname;
    return (
      <div className={"inner55-" + this.props.type + " col-md-12"} style={{ paddingLeft: '8px', marginTop: '-14px' }}>

        <div className="col-xs-12" style={{ padding: '0px', paddingTop: '10px', marginTop: '-10px' }}>
          <label className="col-xs-11 inner55-title" style={{ fontWeight: "bold", paddingLeft: '0px', marginBottom: ' 23px' }}>{title}</label>
        </div>
        {/* <div className="col-xs-12" style={{ padding: '0px'}}>
          <label className="col-xs-4" style={{ paddingLeft: '0px'}}>Tên quỹ</label>
          <div className="col-xs-7">
              

            <InputComponent disabled value={this.state.sessionInfo.DAY} disabled  placeholder="5" validate={this.props.type === "RS" ? this.state.TenQuyMua.validate : this.state.TenQuyBan.validate} type="text" name={this.props.type === "RN" ? "TenQuyMua" : "TenQuyBan"}
        ...
        <h5 className="col-xs-4" style={{ paddingLeft: '0px' }} ><b> {this.props.strings.traddingdate}</b></h5>
          <div className="col-xs-7" style={{ marginTop: '5px' }}>
            ...
            <label id="lbDTDKDay" className="form-control"  >{this.state.sessionInfo.DAY}</label>
            {/* <InputComponent disabled value={this.state.sessionInfo.DAY} disabled  placeholder="5" validate={this.props.type === "RS" ? this.state.TenQuyMua.validate : this.state.TenQuyBan.validate} type="text" name={this.props.type === "RN" ? "TenQuyMua" : "TenQuyBan"}
              tooltip={this.props.type === "mua" ? this.state.TenQuyMua.tooltip : this.state.TenQuyBan.tooltip} onChange={this.handleChange.bind(this)} /> */}
          </div>
        </div>
        ...
        <h5 className="col-xs-4" style={{ paddingLeft: '0px' }} ><b> {this.props.strings.timeclosebook}</b></h5>
          ...
          <div className="col-xs-7" style={{ marginTop: '5px' }}>
            ...
            <label id="lbDTDKClsOrdTime" className="form-control"  >{this.state.sessionInfo.CLSORDTIME}</label>
            {/* <InputComponent disabled value={this.state.sessionInfo.CLSORDTIME} disabled placeholder="15h ngày 15/05/2017" validate={this.props.type === "RN" ? this.state.ThoiGianDongSoLenhMua.validate : this.state.ThoiGianDongSoLenhBan.validate} type="text" name={this.props.type === "RN" ? "ThoiGianDongSoLenhMua" : "ThoiGianDongSoLenhBan"}
        ...
        <h5 className="col-xs-4" style={{ paddingLeft: '0px' }} ><b> {this.props.strings.navbeforsession}</b></h5>
          ...
          <div className="col-xs-7" style={{ marginTop: '5px' }}>
            <NumberInput id="lbDTDKPreNav" decimalScale={2} displayType='text' className="form-control" value={this.state.sessionInfo.PREVNAV} thousandSeparator={true} prefix={''} id="txtPrevnav" />
...
const decorators = flow([
  ...
  translate('ThongTinQuyDTDK')
]);
module.exports = decorators(ThongTinQuyDTDK);
