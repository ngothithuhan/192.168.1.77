...
class TableDongSoLenh extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pages: null,
    ...
    refresh = () => {
        ...
        RestfulUtils.posttrans('/order/getlistcloseorder', { pagesize: this.state.pagesize, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
            if (resData.EC == 0) {
    ...
    async loadData(pagesize, page, keySearch, sortSearch, columns) {
        let that = this;
        await RestfulUtils.post('/order/getlistcloseorder', { pagesize, page, keySearch, sortSearch, language: this.props.language, symbol: this.state.CODEID, OBJNAME: this.props.OBJNAME }).then((resData) => {
...
const decorators = flow([
    ...
    translate('TableDongSoLenh')
]);
module.exports = decorators(TableDongSoLenh);
