...
import { MAXSIZE_PDF } from 'app/Helpers'
...
class ModalUploadFile extends Component {
    ...
    submit() {
        ...
        let { urlPreviewPdf, pdfBase64 } = this.state;
        let isCheckFileImage = (!urlPreviewPdf && !pdfBase64) ? true : false;

        let obj = {
            ACTION: this.props.ACTION,
            CUSTODYCD: this.props.CUSTODYCD,
            TYPE: this.state.upload.TYPE ? this.state.upload.TYPE : "",
            IMGSIGN: isCheckFileImage ? this.state.SIGN_IMG : this.state.pdfBase64,
    ...
    _handleSIGNIMGChange = e => {
        ...
        if (isPDF === true) {
            console.log(MAXSIZE_PDF)
            if (file.size > MAXSIZE_PDF) {
                //console.log('this.props.strings.errorSizePDF:', this.props.strings.errorSizePDF)
                let error = {
                    color: 'red',
                    contentText: this.props.strings.errorSizePDF
                }
                this.setState({ err_msg: error, urlPreviewPDF: '', pdfBase64: '', SIGN_IMG: null })
                return
            } else {
    ...
    render() {
        var self = this;
        let { strings } = this.props;
        let { SIGN_IMG, urlPreviewPDF } = this.state;
            ...
            <Modal show={this.props.showModalDetail} bsSize="modalBigSize">
                        ...
                        <div className="col-md-9 modal-upload">
                            <div className="col-md-12">
                                ...
                                <div className="col-md-9">
                                    <DropdownUtils IsNULL={true}
                                        typeValue="CDVAL"
                                        typeLabel="CDCONTENT"
                                        value={this.state.upload["TYPE"]}
                                        callApi={true}
                                        onChange={this.onChange.bind(this)}
                                        type="TYPE" CDID=""
                                        urlApi="/allcode/getlist"
                                        optionFilter={{
                                            CDNAME: { value: "IMGTYPE", isFilter: true, checkFilter: true },
                                            CDTYPE: { value: "CF", isFilter: true, checkFilter: true }
                                        }} />
                ...
                <Modal.Footer>
                    <div className="col-md-12 btn-customer-model">
                        <button
                            disabled={(SIGN_IMG == null && !urlPreviewPDF) ? true : false}
                            style={{ display: this.props.ACTION == 'VIEW' ? 'none' : 'block', float: 'right' }}
...
const decorators = flow([
    ...
    translate('ModalUploadFile')
]);
module.exports = decorators(ModalUploadFile);
