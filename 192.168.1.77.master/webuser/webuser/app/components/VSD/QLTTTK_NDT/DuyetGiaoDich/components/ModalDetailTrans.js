...
class ModalDetailTrans extends Component {
    ...
    componentWillMount() {
        ...
        RestfulUtils.post('/fund/getlisttx', { data }).then((resData) => {
            ...
            console.log('receiveData', this.state.receiveData)
        });
    }
                        ...
                        <tbody>
                            {receiveData.map((e, index) => {
                                ...
                                return <tr key={index}>
                                    ...
                                    {e.VALUE == e.NVALUE ?
                                        <td colSpan="2"> <NumberFormat className="form-control"  value={e.VALUE} displayType={'text'} thousandSeparator={true} decimalScale={2} /></td> :
                                        <td colSpan="2"><label  className="form-control"  >{e.VALUE != undefined ? e.VALUE : ''}</label></td>
                                    }
                                </tr>
                        ...
                        data.push(
                            {
                             ...
                             //   width:100,
                                accessor: key,
                                Cell: ({ value }) => (
                                    <div className={value != null ? (isNaN(value)==false && ArrSpecial.includes(key) == false && value != '')  ? "col-right" : "col-left": "col-left"}>
                                        {value != null ? (isNaN(value)==false && ArrSpecial.includes(key) == false && value != '')  ? <NumberFormat value={value} displayType={'text'} thousandSeparator={true} decimalScale={2} /> : value : value}
                                    </div>),
    ...
    render() {
        ...
        let displayy = this.props.isApprove == "4" ? "inline-block" : "none"
        ...
        return (
            <div>
                <div className="col-md-12 ">
                    <div className="" style={{ marginBottom: "10px", marginLeft: -13 }}>
                        <button style={{ marginRight: "5px", fontSize: 10, display: displayy }} className="btn btn-primary" onClick={this.approve}><span className="glyphicon glyphicon-ok"></span> {this.props.strings.approvebtn}</button>
                        <button style={{ marginRight: "5px", fontSize: 10, display: displayy }} className="btn btn-default" onClick={this.reject}><span className="glyphicon glyphicon-minus" onClick={this.reject}></span> {this.props.strings.rejectbtn}</button>
