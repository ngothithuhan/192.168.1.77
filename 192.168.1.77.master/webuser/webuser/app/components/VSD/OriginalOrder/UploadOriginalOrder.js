...
import ModalUploadOriginalOrder from './components/ModalUploadOriginalOrder'
...
class UploadOriginalOrder extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  async getListUpload(CUSTODYCD, ACTION) {
    ...
    if (PV_CUSTODYCD && PV_CUSTODYCD != '' && PV_CUSTODYCD != undefined) {
      await RestfulUtils.post('/order/get_list_originalorder', obj).then((resData) => {
        if (resData.EC == 0) {
          self.setState({ 
  ...
  async loadData(pagesize, page, keySearch, sortSearch, columns) {
    let self = this
    let PV_CUSTODYCD = this.state.CUSTODYCD ? this.state.CUSTODYCD.label : ""
    self.setState({ DTUpload: [], selectedRows: new Set() });
    let obj = {
      CUSTODYCD: PV_CUSTODYCD,
      STATUS: "A",
    ...
    if (PV_CUSTODYCD && PV_CUSTODYCD != '' && PV_CUSTODYCD != undefined) {
      await RestfulUtils.post('/order/get_list_originalorder', { ...obj, pagesize, page, keySearch, sortSearch }).then(resData => {
        if (resData.EC === 0) {
          self.setState({
            DTUpload: resData.DT ? resData.DT.data : [],
            pages: resData.numOfPages,
  ...
  render() {
    ...
    return (
      <div style={{ borderColor: "rgba(123, 102, 102, 0.43)", padding: "10px" }} className="container panel panel-success margintopNewUI">
        ...
        <div className="col-xs-12 FixtableRT">
          <ReactTable
            columns={[
              ...
              // },
              {
                Header: props => <div className="">{strings.orderid}</div>,
                accessor: "ORDERID",
                width: 120,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.exectype}</div>,
              ...
              {
                Header: props => <div className="">{strings.orderstatus}</div>,
                accessor: "ORSTATUS",
                width: 110,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.txdate}</div>,
                accessor: "TXDATE",
                width: 100,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.txtime}</div>,
                accessor: "TXTIME",
                width: 75,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.placeuser}</div>,
                accessor: "USERNAME",
                width: 100,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.fundcode}</div>,
                accessor: "SYMBOL",
                width: 100,
                Cell: ({ value }) => (
                  <div className="col-left" style={{ float: "left" }}>{value}</div>
              ...
              {
                Header: props => <div className="">{strings.amount}</div>,
                accessor: "ORDERVALUE",
                width: 100,
                Cell: ({ value }) => (
              ...
              {
                Header: props => <div className="">{strings.matchqtty}</div>,
                accessor: "MATCHQTTY",
                width: 100,
                Cell: ({ value }) => (
                  <span className="col-right">
        ...
        />
        <ModalUploadOriginalOrder OBJNAME={datapage.OBJNAME} ACTION={this.state.ACTIONEditOrDelete}
          DTUploadDetail={DTUploadDetail}
...
UploadOriginalOrder.defaultProps = {
  strings: {
...
const decorators = flow([
  ...
  translate('UploadOriginalOrder')
]);

module.exports = decorators(UploadOriginalOrder);
